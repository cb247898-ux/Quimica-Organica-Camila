...
# -------------------------- CONFIGURACIÓN --------------------------
st.set_page_config(
    page_title="🧪 Calculadora de Pesos Moleculares",
    page_icon="⚗️",
    layout="wide",
...
def convertir_masa_moles(valor, peso_molar, desde_a):
    """Convierte gramos ↔ moles ↔ partículas"""
    N_A = 6.02214076e23
    if desde_a == "g → mol":
        return valor / peso_molar
    elif desde_a == "mol → g":
        return valor * peso_molar
    elif desde_a == "mol → partículas":
        return valor * N_A
    elif desde_a == "g → partículas":
        return (valor / peso_molar) * N_A
    elif desde_a == "partículas → mol":
        return valor / N_A
    elif desde_a == "partículas → g":
        return (valor / N_A) * peso_molar
...
def main():
    # ✅ Título modificado
    st.title("🧪 Calculadora de Pesos Moleculares")
    # ✅ Creadora agregada justo debajo
    st.subheader("Creadora: Camila Rosas")
    st.markdown("Pesos • Composición • Conversiones • Balanceo • Estructura • Gases")
    st.markdown("---")

    tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs([
        "⚗️ Peso y Composición", "⚖️ Conversor Masa-Moles", "🔄 Unidades",
        "📦 Estructura", "⚖️ Balanceo", "💨 Gases y Fórmulas"
    ])

    # ---------- PESTAÑA 1: PESO Y COMPOSICIÓN PORCENTUAL ----------
    with tab1:
    ...
    # ---------- PESTAÑA 2: CONVERSOR GRAMOS ↔ MOLES ↔ PARTÍCULAS ----------
    with tab2:
        st.subheader("⚖️ Conversor Masa ↔ Cantidad de sustancia")
        st.info("Primero calcula el peso molecular en la pestaña anterior o ingrésalo manualmente")
    ...
    # ---------- PESTAÑA 3: CONVERSIÓN DE UNIDADES ----------
    with tab3:
    ...
    # ---------- PESTAÑA 4: ESTRUCTURA QUÍMICA INTERACTIVA ----------
    with tab4:
        ...
        if st.button("Mostrar estructura"):
            ...
            if not e:
                ...
                st.caption("💡 Para ver estructuras 3D interactivas en la nube, agrega la librería `rdkit` a requirements.txt")
    ...
    st.markdown("---")
    st.caption("🧪 Versión completa con todas las funcionalidades solicitadas")
